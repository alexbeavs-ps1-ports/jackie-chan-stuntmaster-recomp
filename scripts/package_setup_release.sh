...
#
# Writes: dist/jackie-chan-stuntmaster-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix jackie-chan-stuntmaster-recomp \
  --exe-name Jackie_Chan_Stuntmaster_Recompiled \
  --display-name "Jackie Chan Stuntmaster Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Jackie Chan Stuntmaster disc" \
